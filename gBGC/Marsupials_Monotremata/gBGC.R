...
require(reshape2)
d_m <- melt(d3)
g<- ggplot() + geom_line(data = d_m, aes(x = Var1, y = value, group = Var2,colour=factor(Var2)),size=5)+theme(legend.text=element_text(face="italic"),legend.title=element_text(size=30))+ scale_color_discrete(name="Species") + labs(tag = "G")+ ggtitle("gBGC vs Nucleotide position ")+theme(plot.title = element_text(hjust = 0.5,color="red", size=54, face="bold"),axis.title.x = element_text(color="blue", size=54, face="bold"),axis.title.y = element_text(color="blue", size=54, face="bold"),axis.text=element_text(size=52),legend.text=element_text(size=30)) + xlab("Nucleotide position ") + ylab("gBGC") + guides(size=FALSE) + theme(plot.margin = unit(c(2,2,2,2), "lines"),text = element_text(size=40))
